# Ciclo de trabalho na branch `back-end`

Este guia é para quem vai trabalhar na branch `back-end` e precisa seguir o ciclo completo com clareza. A ideia é sempre trabalhar em `back-end`, enviar os commits para o GitHub, abrir o pull request, fazer o merge e voltar para `back-end` para continuar a próxima etapa.
...
Depois disso, entre na branch `back-end`.
...
4. Envie a branch `back-end` para o GitHub.
...
Depois de enviar os commits, abra o pull request da `back-end` para `main` no GitHub web.
...
- A branch `back-end` já foi enviada para o remoto.
...
Em seguida, volte para `back-end` para continuar a próxima etapa.
...
A branch `back-end` deve ser mantida durante todo o projeto. A cada novo conjunto de arquivos, você repete o ciclo de commit, push, PR, merge e retorno para `back-end`.